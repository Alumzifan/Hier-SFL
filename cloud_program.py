...
from data_preprocess import attacks
from setup import prRed, prGreen, num_clients, device, lr, epochs
...
def evaluate_cloud(fx_client, y, idx, len_batch, batch_idx, ell, idx_count):
    global net_model_cloud, criterion, batch_acc_test, batch_loss_test, check_fed, net_cloud, net_glob_cloud, y_true
    ...
    global loss_test_collect_user, acc_test_collect_user, acc_avg_all_user_train, loss_avg_all_user_train, y_pred
    ...
    with torch.no_grad():
        ...
        # show some metrics in the end
        if ell == epochs - 1:
            for i in y:
                y_true.append(attacks[i])
            for i in fx_cloud:
                i = i.tolist()
                i_index = i.index(max(i))
                y_pred.append(attacks[i_index])
            if idx_count == num_clients and batch_idx == len_batch - 1:
                print('------Metrics------')
                print('accuracy:', accuracy_score(y_true, y_pred))
                print('precision:', precision_score(y_true, y_pred, average='weighted'))
                print('recall:', recall_score(y_true, y_pred, average='weighted'))
                print('f1-score:', f1_score(y_true, y_pred, average='weighted'))
                C = confusion_matrix(y_true, y_pred, labels=attacks)
                print(attacks)
                print(C)
                df = pd.DataFrame(C, index=attacks, columns=attacks)
