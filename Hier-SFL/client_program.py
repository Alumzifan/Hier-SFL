...
from edge_model import net_glob_edge
...
# init two edges
Edge1 = Edge(net_glob_edge, client_ep, lr, device)
Edge2 = Edge(net_glob_edge, client_ep, lr, device)

# Client-side functions associated with Training and Testing
class Client(object):
    def __init__(self, net_client_model, client_ep, idx, lr, device, batch_size, edge_flag, dataset_train=None, dataset_test=None, idxs=None,
                 idxs_test=None):
        self.idx = idx
        self.device = device
        self.lr = lr
        ...
        self.ldr_train = DataLoader(DatasetSplit(dataset_train, idxs), batch_size=batch_size, shuffle=True, pin_memory=True)
        self.ldr_test = DataLoader(DatasetSplit(dataset_test, idxs_test), batch_size=batch_size, shuffle=True, pin_memory=True)
        self.client_ep_time = []
        self.edge_ep_time = []
    ...
    def train(self, global_ep, net, net_auxiliary):
        ...
        for iter in range(self.client_ep):
            ...
            for batch_idx, (features, labels) in enumerate(self.ldr_train):
                ...
                features = torch.reshape(features, [-1, 1, 32])
                ...
                if (iter + 1) % 10 == 0 and iter != self.client_ep - 1:
                    ...
                    if self.edge_flag == 1:
                        w_edge, w_edge_auxiliary, edge_fx, edge_loss = Edge1.train_edge(global_ep, client_fx, labels, iter, self.client_ep, self.idx, len_batch)
                    else:
                        w_edge, w_edge_auxiliary, edge_fx, edge_loss = Edge2.train_edge(global_ep, client_fx, labels, iter, self.client_ep, self.idx, len_batch)
                ...
                if (iter + 1) % 10 == 0 and iter == self.client_ep - 1:
                    ...
                    if len_count != len_batch:
                        ...
                        return net.state_dict(), net_auxiliary.state_dict(), w_edge, w_edge_auxiliary, \
    ...
    def evaluate(self, net, ell):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (features, labels) in enumerate(self.ldr_test):
                ...
                # Sending activations to edge
                if self.edge_flag == 1:
                    Edge1.evaluate_edge(fx, labels, self.idx, len_batch, ell)
                else:
                    Edge2.evaluate_edge(fx, labels, self.idx, len_batch, ell)
