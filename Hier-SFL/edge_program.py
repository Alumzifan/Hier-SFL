...
class Edge(object):
    def __init__(self, net_edge_model, client_ep, lr, device):
        self.device = device
        self.lr = lr
        ...
        self.w_glob_edge = net_glob_edge.state_dict()
        self.w_glob_edge_auxiliary = net_glob_edge_auxiliary.state_dict()
        self.w_locals_edge = []
        ...
        self.net_model_edge = [net_glob_edge for i in range(num_clients)]
        self.net_model_edge_auxiliary = [net_glob_edge_auxiliary for j in range(num_clients)]
    ...
    def train_edge(self, global_ep, fx_client, y, l_epoch_count, l_epoch, idx, len_batch):
        ...
        if self.count1 == len_batch:
            ...
            if len(self.idx_collect) == num_clients:
                self.fed_check = True  # to evaluate_server function  - to check fed check has hitted
        ...
        return net_edge.state_dict(), net_edge_auxiliary.state_dict(), edge_fx, local_loss.item()

    def evaluate_edge(self, fx_client, y, idx, len_batch, ell):
        ...
        with torch.no_grad():
            ...
            evaluate_cloud(fx_edge, y, idx, len_batch, ell)
