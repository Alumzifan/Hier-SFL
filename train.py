...
from data_preprocess import dataset_train, dataset_test, dict_users, dict_users_test, \
    dict_users_non_iid, dict_users_test_non_iid
from cloud_program import FedAvg, acc_train_collect, acc_test_collect, loss_train_collect, loss_test_collect
from setup import epochs, frac, num_clients, lr, device, program, client_ep
...
for iter in range(epochs):
    ...
    for idx in idxs_users:
        ...
        if idx == 0 or idx == 1:
            ...
            edge_flag = 2
        local = Client(net_glob_client, client_ep, idx, lr, device, edge_flag, dataset_train=dataset_train, dataset_test=dataset_test,
                       idxs=dict_users[idx], idxs_test=dict_users_test[idx])
        # Training ------------------
        w_client, w_client_auxiliary, client_running_time, edge_running_time, \
        ...
        # Testing -------------------
        local.evaluate(net=copy.deepcopy(net_glob_client).to(device), ell=iter, idx_count=idx_count)
