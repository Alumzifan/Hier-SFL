...
class ResNet18_cloud_side(nn.Module):
    def __init__(self, block, num_layers, classes):
        super(ResNet18_cloud_side, self).__init__()
        self.inplanes = 64
    ...
    def forward(self, x):
        # x = self.layer2(x)
        # x = x + out1  # adding the resudial inputs -- downsampling not required in this layer
        # x = self.layer3(x)
        # x = self.layer4(x)
        # x6 = self.layer6(x5)
...
net_glob_cloud = ResNet18_cloud_side(BasicBlock, [1, 1], 14)  # 14 is my numbr of classes
...
cloud_side_model = ResNet18_cloud_side(BasicBlock, [1, 1], 14).to(device)
# 传入model和输入数据的shape
summary(cloud_side_model, (64, 17))